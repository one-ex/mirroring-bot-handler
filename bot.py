...
def format_job_progress(job_info: dict, status_info: dict, bot_username: str = None) -> str:
    ...
    if status in ['Completed', 'Sukses']:
        ...
        # Menggunakan command /stop dengan parameter job_id
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.args and context.args[0].startswith('stop_'):
        ...
        await stop_command_with_id(update, context, job_id)
    else:
...
async def stop_command_with_id(update: Update, context: ContextTypes.DEFAULT_TYPE, job_id: str) -> None:
    """Fungsi internal untuk memproses pembatalan job dengan job_id."""
    user = update.effective_user
...
def setup_bot():
    ...
    # Handler untuk /start (juga menangani deep link)
