...
async def update_progress(context) -> None:
    ...
    for service, base_url in service_urls.items():
        ...
        tasks.append(client.get(f"{base_url}/status/all", timeout=10))
    ...
    if not context.bot_data.get('active_mirrors'):
        jobs = context.application.job_queue.get_jobs_by_name('update_progress')
        for job in jobs:
